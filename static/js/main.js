...
// ==============================================================
// LICENSE EXPIRY OVERLAY - SALES INTERFACE ONLY
// Implementation per LICENSE-OVERLAY-20260429-FINAL
// ==============================================================

// Use window object to avoid duplicate declaration errors
if (typeof window.licenseOverlayActive === 'undefined') {
    window.licenseOverlayActive = false;
    window.licensePollingInterval = null;
}

function initLicenseMonitoring() {
    // Only run this in sales interface
    if (!document.getElementById('sale_container')) {
        return;
    }
    
    console.log('✅ License monitoring initialized for sales interface');
    
    // Create hidden license card element
    createLicenseCard();
    
    // Start polling every 60 seconds
    checkLicenseStatus();
    licensePollingInterval = setInterval(checkLicenseStatus, 60000);
}

function createLicenseCard() {
    // Create license card (hidden by default)
    const card = document.createElement('div');
    card.id = 'license_status_card';
...
function checkLicenseStatus() {
    ...
    .then(data => {
        if (data.app_state === 'expired') {
            activateLicenseOverlay(data.days_overdue || 0);
        } else {
            deactivateLicenseOverlay();
        }
    })
    .catch(err => {
        console.log('⚠️ License check failed, will retry');
    });
}

function activateLicenseOverlay(daysOverdue) {
    if (licenseOverlayActive) return;
    
    licenseOverlayActive = true;
    console.log(`🔒 License expired ${daysOverdue} days ago - activating overlay`);
    
    // Create dark overlay background
    const overlay = document.createElement('div');
    overlay.id = 'license_expiry_overlay';
    ...
    // Show and center license card
    const card = document.getElementById('license_status_card');
    ...
    card.innerHTML = `
        <div style="font-size: 24px; margin-bottom: 15px;">⚠️ LICENSE EXPIRED</div>
        <div style="font-size: 18px; margin-bottom: 10px;">${daysOverdue} DAYS OVERDUE</div>
        <div style="font-size: 14px; opacity: 0.9;">System is locked until license renewal</div>
    `;
    
    // Disable all sales interface elements
    const salesContainer = document.getElementById('sale_container');
    if (salesContainer) {
        salesContainer.style.pointerEvents = 'none';
        salesContainer.style.opacity = '0.4';
    }
}

function deactivateLicenseOverlay() {
    if (!licenseOverlayActive) return;
    
    licenseOverlayActive = false;
    console.log('✅ License active - removing overlay');
    
    // Remove overlay
    const overlay = document.getElementById('license_expiry_overlay');
    ...
    // Re-enable sales interface
    const salesContainer = document.getElementById('sale_container');
    if (salesContainer) {
        salesContainer.style.pointerEvents = 'auto';
        salesContainer.style.opacity = '1';
    }
}

// Initialize on page load
document.addEventListener('DOMContentLoaded', initLicenseMonitoring);

// ==============================================================
// END LICENSE EXPIRY OVERLAY
// ==============================================================
